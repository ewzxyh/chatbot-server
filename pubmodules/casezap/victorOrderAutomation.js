...
function buildQuoteNotification(options) {
  return 'Cotação enviada para o cliente ' + (options.phone || 'desconhecido') +
    ': R$ ' + formatAmountCents(options.amountCents) + '. Aguardando comprovante para conferência manual.';
}
...
async function handleInboundMessage(options) {
  ...
  var mappedPhone = String(options.mapped && options.mapped.phone || '').replace(/\D/g, '');
  ...
  if (fromMe === true) {
    ...
    await notifyVictorNumbers({
      ...
      text: buildQuoteNotification({ phone: mappedPhone, amountCents: amountCents }),
      sendInternalMessage: options.sendInternalMessage
    });
    return { status: 'quoted', amountCents: amountCents };
  }

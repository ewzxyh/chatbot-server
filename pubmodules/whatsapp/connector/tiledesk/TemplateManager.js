...
class TemplateManager {
  ...
  constructor(config) {
    ...
    if (!config.waba_id) {
      throw new Error('config.waba_id is mandatory');
    }
  ...
  async getTemplates() {
    return await axios({
      url: this.graph_url + this.waba_id + "/message_templates?access_token=" + this.token,
      headers: {
  ...
  async getTemplateNamespace() {
    ...
    return await axios({
      url: this.graph_url + this.waba_id + "?fields=message_template_namespace&access_token=" + this.token,
      headers: {
        'Content-Type': 'application/json'
      },
      method: 'GET'
